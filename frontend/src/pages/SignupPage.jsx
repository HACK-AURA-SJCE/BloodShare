...
// }
import 'leaflet/dist/leaflet.css';
import { CreditCard, Heart, Home, Hospital, Lock, Mail, MapPin, Phone, User } from 'lucide-react';
import { useEffect, useRef, useState } from 'react';
import { MapContainer, Marker, TileLayer, useMap, useMapEvents } from 'react-leaflet';
import { Link, useNavigate } from 'react-router-dom';
import { useAuth } from '../context/AuthContext';
...
const SignupPage = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-red-50 via-rose-50 to-pink-50 py-20 px-4 pt-24">
      <div className="max-w-3xl mx-auto">
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8 mb-8 border-t-4 border-red-600">
          <div className="text-center">
            ...
            <h1 className="text-4xl font-bold text-gray-900 mb-2">Join BloodConnect</h1>
            <p className="text-gray-600 text-lg">Be a part of something life-saving</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-2xl p-8 border border-gray-100">
          ...
          {/* Toggle Buttons */}
          <div className="flex bg-gradient-to-r from-gray-50 to-gray-100 rounded-xl p-1.5 mb-8 shadow-inner">
            <button
              type="button"
              onClick={() => setSignupType('donor')}
              className={`flex-1 py-4 px-6 rounded-lg font-semibold transition-all duration-300 flex items-center justify-center gap-3 ${signupType === 'donor'
                  ? 'bg-gradient-to-r from-red-600 to-red-700 text-white shadow-lg transform scale-105'
                  : 'text-gray-600 hover:text-gray-900 hover:bg-white'
                }`}
            >
              <Heart className={`w-5 h-5 ${signupType === 'donor' ? 'fill-white' : ''}`} />
              <span>I'm a Donor</span>
            </button>
            <button
              type="button"
              onClick={() => setSignupType('hospital')}
              className={`flex-1 py-4 px-6 rounded-lg font-semibold transition-all duration-300 flex items-center justify-center gap-3 ${signupType === 'hospital'
                  ? 'bg-gradient-to-r from-green-600 to-green-700 text-white shadow-lg transform scale-105'
                  : 'text-gray-600 hover:text-gray-900 hover:bg-white'
                }`}
            >
              <Hospital className="w-5 h-5" />
              <span>I'm a Hospital</span>
            </button>
          </div>
          ...
          <div onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="space-y-6">
                  <div className="grid md:grid-cols-2 gap-6">
                    <div className="relative">
                      ...
                      <input
                        className="w-full pl-11 pr-4 py-3 border-2 border-gray-300 rounded-lg focus:border-red-500 focus:ring-2 focus:ring-red-200 outline-none transition-all"
                  ...
                  <div className="grid md:grid-cols-2 gap-6">
                    ...
                    <div className="relative">
                      <span className="absolute left-3 top-1/2 transform -translate-y-1/2 text-red-600 font-bold text-lg">🩸</span>
                      <select
                        className="w-full pl-11 pr-4 py-3 border-2 border-gray-300 rounded-lg focus:border-red-500 focus:ring-2 focus:ring-red-200 outline-none transition-all appearance-none bg-white"
                        required
                  ...
                  <div className="relative">
                    <CreditCard className="absolute left-3 top-4 w-5 h-5 text-gray-400" />
                    <input
                      className="w-full pl-11 pr-4 py-3 border-2 border-gray-300 rounded-lg focus:border-red-500 focus:ring-2 focus:ring-red-200 outline-none transition-all"
                      placeholder="Aadhar Number"
                      required
                      value={donorData.aadhar}
                      onChange={(e) => setDonorData({ ...donorData, aadhar: e.target.value })}
                    />
                  </div>
                  ...
                  <div className="relative">
                    ...
                    <textarea
                      className="w-full pl-11 pr-4 py-3 border-2 border-gray-300 rounded-lg focus:border-red-500 focus:ring-2 focus:ring-red-200 outline-none transition-all resize-none"
                  ...
                  <div>
                    ...
                    <div className="h-72 rounded-xl overflow-hidden border-2 border-gray-300 shadow-lg">
                      <MapPicker
                        center={center}
                        value={picked}
                        onChange={(lat, lng) => {
                          setPicked([lat, lng]);
                          setDonorData({ ...donorData, lat: String(lat), lng: String(lng) });
                        }}
                        address={donorData.address}
                      />
                    </div>
                    {picked && (
                      <p className="mt-2 text-sm text-green-600 flex items-center gap-1">
                        <span>✓</span> Location selected: {picked[0].toFixed(4)}, {picked[1].toFixed(4)}
                      </p>
                    )}
                  </div>
              ...
              <>
                ...
                <div className="space-y-6">
                  <div className="relative">
                    ...
                    <input
                      className="w-full pl-11 pr-4 py-3 border-2 border-gray-300 rounded-lg focus:border-green-500 focus:ring-2 focus:ring-green-200 outline-none transition-all"
                  ...
                  <div className="relative">
                    <Home className="absolute left-3 top-4 w-5 h-5 text-gray-400" />
                    <textarea
                      className="w-full pl-11 pr-4 py-3 border-2 border-gray-300 rounded-lg focus:border-green-500 focus:ring-2 focus:ring-green-200 outline-none transition-all resize-none"
                  ...
                  <div>
                    ...
                    <div className="h-72 rounded-xl overflow-hidden border-2 border-gray-300 shadow-lg">
                      <MapPicker
                        ...
                        onChange={(lat, lng) => {
                          setPicked([lat, lng]);
                          setHospitalData({ ...hospitalData, lat: String(lat), lng: String(lng) });
                        }}
                        address={hospitalData.address}
                      />
                    </div>
            ...
            <button
              ...
              disabled={loading}
              className={`w-full py-4 rounded-xl font-bold text-lg shadow-lg transition-all transform ${loading
        ...
        {/* Footer */}
        <div className="mt-8 text-center bg-white rounded-xl shadow-md p-6">
